...
int main(int argc, char *argv[]){
    if((FDlog = open("log/processA.log",O_WRONLY|O_CREAT|O_APPEND, 0666))==-1){
        perror("Error opening process A log file.");
        exit(1);
    }

    WriteLog("Process A started.");
    ...
    while (TRUE){
        ...
        if(cmd == KEY_RESIZE){
            ...
            WriteLog("Keyboard arrow buttom is pressed.");

            move_circle(cmd);
            draw_circle();

            //Delete the blue circle
            BlueCircle(radius, x, y, bmp, false);
            ...
            BlueCircle(radius, circle.x, circle.y, bmp, true);
